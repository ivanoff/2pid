...
describe('two applications with one pid trying', function() {
  ...
  var PID_FILE = '/tmp/2pidtestForceRemove.pid';
  ...
  describe('first application ends without close', function() {
    it('try to remove pid', function(done) {
      pid.status.should.not.equal('stopped');
      pid.status.should.equal('started');
      done();
    });
  ...
  describe('second application try to force remove pid file', function() {
    var pid2 = new Pid(PID_FILE);

    it('try to remove pid', function(done) {
      pid2.remove({force: true});
      pid2.status.should.equal('stopped');
      done();
    });
